...
df_anime = pd.concat([df_anime, df_anime['subtype'].str.get_dummies(), df_anime['genre'].str.get_dummies(sep=',')], axis=1)
...
def recommend_anime(title, type, similarity=cosine_sim):
    ...
    if df_anime['title'].str.contains(title).sum() > 0:
        ...
        recommendation = df_anime[['title', 'status' ,'subtype','epCount','image','rating','synopsis']].iloc[anime_indices]
